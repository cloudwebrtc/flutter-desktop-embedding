...
//    REVISION HISTORY
//
//      2005-JAN-10 - Thanks to Don Beusee for pointing out the danger in
//      mapping
...
//                    compiler warnings (and yes, even a couple of actual
//                    compiler errors).  These include Henk Demper for figuring
//                    out how to make the Intellisense work on with CStdString
//                    on VC6, something I was never able to do.  Greg Marr
//                    pointed out a compiler warning about an unreferenced
//                    symbol and a problem with my version of Load in MFC
//                    builds.  Bill Carducci took a lot of time with me to help
//                    me figure out why some implementations of the Standard C++
//                    Library were returning error codes for apparently
//                    successful conversions between ASCII and UNICODE.  Finally
//                    thanks to Brian Groose for helping me fix compiler signed
//                    unsigned warnings in several functions.
//
//    2003-JUL-10 - Thanks to Charles Godwin for making me realize my 'FmtArg'
//                    fixes had inadvertently broken the DLL-export code (which
//                    is
...
//
//      2002-FEB-25 - Thanks to Tim Dowty for pointing out (and giving me the
//      fix
//                    for) a conversion problem with non-ASCII MBCS characters.
//                    CStdString is now used in my favorite commercial MP3
//                    player!
//
//      2001-DEC-06 - Thanks to Wang Haifeng for spotting a problem in one of
//      the
//                    assignment operators (for _bstr_t) that would cause
//                    compiler errors when refcounting protection was turned
//                    off.
//
...
//                    because you're not supposed to inject your own code into
//                    the 'std' namespace.  But if you don't care about that,
//                    it's there if you want it
//                  - Thanks to Jason Mills for catching a case where CString
//                  was
...
//                  - Joe also caught me truncating strings for GetBuffer()
//                  calls
...
//                  - Got rid of the NSP macro - it interfered with Koenig
//                  lookup
...
//                  - Thanks to Jeff Kohn for prompting me to find and fix a
//                  typo
...
//                    longer than 31 characters, get a pointer to it (via
//                    c_str()) and then assign that pointer to the original
//                    string object. The resulting string would be empty.  Not
//                    with CStdString!
//
//      1999-OCT-06 - BufferSet was erasing the string even when it was merely
//                    supposed to shrink it.  Fixed.  Thanks to Chris Conti.
//                  - Some of the Q172398 fixes were not checking for
//                  assignment-
...
//
//      1999-JUL-21 - Made all calls to GetBuf() with no args check length
//      first.
//
...
//                    reference-counted (in those implementations that
//                    refcount).
//
//      1999-JUL-01 - Added a true CString facade.  Now you can use CStdString
//      as
//                    a drop-in replacement for CString.  If you find this
//                    useful, you can thank Chris Sells for finally convincing
//                    me to give in and implement it.
//                  - Changed operators << and >> (for MFC CArchive) to
//                  serialize
//                    EXACTLY as CString's do.  So now you can send a CString
//                    out to a CArchive and later read it in as a CStdString. I
//                    have no idea why you would want to do this but you can.
//
...
//                  - Added equivalents of CString::GetBuffer,
//                  GetBufferSetLength
//                  - new sscpy() replacement of CStdString::CopyString()
//                  - a Trim() function that combines TrimRight() and
//                  TrimLeft().
//
//      1999-MAR-13 - Corrected the "NotSpace" functional object to use
//      _istpace()
...
//                    originally assumed that these were unnecessary and would
//                    be inherited from basic_string.  However, without them,
//                    Visual C++ complains about ambiguous overloads when you
//                    try to use them.  Thanks to Julian Selman to pointing this
//                    out.
//
...
// SS_IS_INTRESOURCE
// -----------------
//        A copy of IS_INTRESOURCE from VC7.  Because old VC6 version of
//        winuser.h doesn't have this.
...
//        allocation of more space for wchar_t --> char conversions. Note that
//        MBCS does not mean UNICODE.
//
//    #define SS_MBCS
//
...
inline PWSTR StdCodeCvt(PWSTR pDstW,
                        int nDst,
                        PCSTR pSrcA,
                        int nSrc,
                        UINT acp = CP_ACP) {
...
inline PSTR StdCodeCvt(PSTR pDstA,
                       int nDst,
                       PCWSTR pSrcW,
                       int nSrc,
                       UINT acp = CP_ACP) {
...
inline PUSTR StdCodeCvt(PUSTR pDstA,
                        int nDst,
                        PCWSTR pSrcW,
                        int nSrc,
                        UINT acp = CP_ACP) {
...
inline PWSTR StdCodeCvt(PWSTR pDstW,
  ...
  if (nSrc > 0) {
    ...
    res = conv.in(st, pSrcA, pSrcA + nSrc, pNextSrcA, pDstW, pDstW + nDst,
                  pNextDstW);
...
inline PSTR StdCodeCvt(PSTR pDstA,
  ...
  if (nSrc > 0) {
    ...
    const SSCodeCvt& conv = SS_USE_FACET(loc, SSCodeCvt);
    SSCodeCvt::state_type st = {0};
    res = conv.out(st, pSrcW, pSrcW + nSrc, pNextSrcW, pDstA, pDstA + nDst,
                   pNextDstA);
...
#define SSA2W(pa)                                                         \
  (((_pa = pa) == 0)                                                      \
       ? 0                                                                \
       : (_cvt = (sslen(_pa)), StdCodeCvt((PWSTR)_alloca((_cvt + 1) * 2), \
                                          (_cvt + 1) * 2, _pa, _cvt, _acp)))
#define SSW2A(pw)                                                       \
  (((_pw = pw) == 0)                                                    \
       ? 0                                                              \
       : (_cvt = sslen(_pw), StdCodeCvt((LPSTR)_alloca((_cvt + 1) * 2), \
                                        (_cvt + 1) * 2, _pw, _cvt, _acp)))
#else
...
// --------------------------------------------------------------------------
inline bool wsGetStringType(LCID lc,
                            DWORD dwT,
                            PCSTR pS,
                            int nSize,
                            WORD* pWd) {
...
inline void ssasn(std::wstring& sDst, PCSTR pA) {
  ...
  if (0 == nSrc) {
    ...
    sDst.resize(nDst + 1);
    PCWSTR szCvt =
        StdCodeCvt(const_cast<SW_PTRTYPE>(sDst.data()), nDst, pA, nSrc);
...
inline void ssadd(std::string& sDst, const std::wstring& sSrc) {
  ...
  if (nSrc > 0) {
    ...
    PCSTR szCvt = StdCodeCvt(const_cast<SS_PTRTYPE>(sDst.data() + nDst), nAdd,
                             sSrc.c_str(), nSrc);
...
inline void ssadd(std::wstring& sDst, const std::string& sSrc) {
  if (!sSrc.empty()) {
    ...
    PCWSTR szCvt = StdCodeCvt(const_cast<SW_PTRTYPE>(sDst.data() + nDst), nSrc,
                              sSrc.c_str(), nSrc + 1);
...
inline int ssvsprintf(PWSTR pW, size_t nCount, PCWSTR pFmtW, va_list vl) {
  ...
  //    #if !defined(__MWERKS__) && !defined(__SUNPRO_CC_COMPAT) &&
  //    !defined(__SUNPRO_CC)
...
inline int ssicoll(const CT* sz1, int nLen1, const CT* sz2, int nLen2) {
  ...
  return coll.compare(s2.c_str(), s2.c_str() + nLen2, s1.c_str(),
                      s1.c_str() + nLen1);
}
#endif
...
// -----------------------------------------------------------------------------
#if defined(SS_WIN32) && !defined(SS_ANSI)
inline DWORD ssfmtmsg(DWORD dwFlags,
                      LPCVOID pSrc,
                      DWORD dwMsgId,
                      DWORD dwLangId,
                      PSTR pBuf,
...
//
//        Note that although this is a template, it makes the assumption that
//        the template argument (CT, the character type) is either char or
//        wchar_t.
// =============================================================================
...
#define SSRES(pctstr) LOWORD(reinterpret_cast<unsigned long>(pctstr))
  ...
  CStdStr(PCMYSTR pT, MYSIZE n)
      : MYBASE(NULL == pT ? MYTYPE().c_str() : pT, n) {}
...
// They also are compiled if you have explicitly turned off refcounting
  ...
  void RelBuf(int nNewLen = -1) {
    this->resize(
        static_cast<MYSIZE>(nNewLen > -1 ? nNewLen : sslen(this->c_str())));
  }
  ...
  void Format(UINT nId,
    ...
    if (strFmt.Load(nId)) {
      Fmt(strFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
          FmtArg<A4>(v4)(), FmtArg<A5>(v5)(), FmtArg<A6>(v6)());
    }
  }
  template <class A1,
            class A2,
  ...
  void Format(UINT nId,
    ...
    if (strFmt.Load(nId)) {
      Fmt(strFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
          FmtArg<A4>(v4)(), FmtArg<A5>(v5)(), FmtArg<A6>(v6)(),
          FmtArg<A7>(v7)(), FmtArg<A8>(v8)(), FmtArg<A9>(v9)());
    }
  }
  template <class A1,
            class A2,
  ...
  void Format(UINT nId,
    ...
    if (strFmt.Load(nId)) {
      Fmt(strFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
          FmtArg<A4>(v4)(), FmtArg<A5>(v5)(), FmtArg<A6>(v6)(),
          FmtArg<A7>(v7)(), FmtArg<A8>(v8)(), FmtArg<A9>(v9)(),
          FmtArg<A10>(v10)(), FmtArg<A11>(v11)(), FmtArg<A12>(v12)());
    }
  }
  template <class A1,
            class A2,
  ...
  void Format(UINT nId,
    ...
    if (strFmt.Load(nId)) {
      Fmt(strFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
          ...
          FmtArg<A7>(v7)(), FmtArg<A8>(v8)(), FmtArg<A9>(v9)(),
          FmtArg<A10>(v10)(), FmtArg<A11>(v11)(), FmtArg<A12>(v12)(),
          FmtArg<A13>(v13)(), FmtArg<A14>(v14)(), FmtArg<A15>(v15)());
    }
  }
  template <class A1,
            class A2,
  ...
  void Format(const CT* szFmt,
              ...
              const A7& v7) {
    Fmt(szFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
        FmtArg<A4>(v4)(), FmtArg<A5>(v5)(), FmtArg<A6>(v6)(), FmtArg<A7>(v7)());
  }
  template <class A1,
            class A2,
            class A3,
  ...
  void Format(const CT* szFmt,
    ...
    Fmt(szFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
        FmtArg<A4>(v4)(), FmtArg<A5>(v5)(), FmtArg<A6>(v6)(), FmtArg<A7>(v7)(),
        FmtArg<A8>(v8)(), FmtArg<A9>(v9)(), FmtArg<A10>(v10)(),
        FmtArg<A11>(v11)(), FmtArg<A12>(v12)(), FmtArg<A13>(v13)());
  }
  template <class A1,
            class A2,
            class A3,
  ...
  void Format(const CT* szFmt,
    ...
    Fmt(szFmt, FmtArg<A1>(v1)(), FmtArg<A2>(v2)(), FmtArg<A3>(v3)(),
        ...
        FmtArg<A8>(v8)(), FmtArg<A9>(v9)(), FmtArg<A10>(v10)(),
        FmtArg<A11>(v11)(), FmtArg<A12>(v12)(), FmtArg<A13>(v13)(),
        FmtArg<A14>(v14)(), FmtArg<A15>(v15)(), FmtArg<A16>(v16)());
  }
  template <class A1,
            class A2,
            class A3,
...
// -------------------------------------------------------------------------
  ...
  //    CT*                LockBuffer()    { return GetBuf(); }// won't really
  //    lock void            UnlockBuffer(); { }    // why have UnlockBuffer w/o
  //    LockBuffer?
  ...
  const CT& operator[](int nIdx) const {
    return static_cast<const MYBASE*>(this)->operator[](
...
inline CStdStringA WUSysMessageA(DWORD dwError, DWORD dwLangId = SS_DEFLANGID) {
  ...
  if (0 != ::FormatMessageA(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError, dwLangId,
                            szBuf, 511, NULL))
...
inline CStdStringW WUSysMessageW(DWORD dwError, DWORD dwLangId = SS_DEFLANGID) {
  ...
  if (0 != ::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM, NULL, dwError, dwLangId,
                            szBuf, 511, NULL))
...
//        collections and algorithms) that perform case-insensitive comparisons
//        of CStdString objects.  This is useful for maps in which the key may
//        be the
...
struct StdStringLessNoCaseW
  ...
  inline bool operator()(const CStdStringW& sLeft,
